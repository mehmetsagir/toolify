...
export const Status: React.FC<StatusProps> = ({ status, audioLevel = 0, shortcut = 'Shift+Command+Space', onRecordToggle, onOpenSettings }) => {
  // Calculate scale based on audio level (1.0 to 1.1 for subtle effect)
    ...
    <div className="h-full flex flex-col bg-zinc-950 text-white relative overflow-hidden">
      ...
      <div className="flex-1 flex flex-col items-center justify-center space-y-8 z-10">
        ...
        <div className="text-center space-y-2">
          <h1 className={`text-2xl font-medium tracking-tight transition-all duration-300 ${
            ...
            {status === 'idle' && (
              <div className="flex items-center gap-2 text-zinc-500 text-xs bg-white/5 px-3 py-1 rounded-full border border-white/5">
                {shortcutKeys.map((key, index) => (
                  <span key={index} className="font-mono text-[10px]">{key}</span>
                ))}
              </div>
            )}
            {status === 'recording' && (
